!  DEMInterpolation.f90 
!
!  FUNCTIONS:
!	DEMInterpolation      - Entry point of console application.
!
!
...
!
!  PURPOSE:  Interpolate the DEM data into the mesh files. Only read one DEM 
!            file.
!****************************************************************************
	...
	real*8 radius
	integer pointsClose(100000) !the DEM point which is closest to current mesh node
    integer upLevel
	...
	open(2,FILE='ctr.dat',STATUS='UNKNOWN')
    read(2,*) 
	read(2,*) demfilename
	read(2,*)
	read(2,*) meshfiletype
	...
	open(2,FILE=demfilename,STATUS='UNKNOWN')
    
    read(2,*) nDEMPoints
    
    allocate(points(nDEMPoints,3))

	do i=1,nDEMPoints
	  read(2,*) points(i,1),points(i,2),points(i,3)
!	  read(2,*) tmp,points(i,1),points(i,2),points(i,3)
!      read(2,*) points(i,1),points(i,2),tmp,tmp,points(i,3)
	enddo
...
!$omp end parallel do
    ...
    write(9,*) 'TITLE = "Bottom Elevation"'
	write(9,*) 'VARIABLES = "X", "Y", "Z"'

	!write(9, *) 'ZONE, DATAPACKING=POINT, N=', nNodes, ',E=', nFaces,' ,ZONETYPE=FETRIANGLE'
    write(9, *) 'ZONE T="Zone1", F=FEPOINT, N=', nNodes, ',E=', nFaces,' ,ET=QUADRILATERAL'
    write(9,*) 'DT=(DOUBLE DOUBLE DOUBLE)'
    ...
    if(meshfiletype.eq.1) then 
		...
		if(meshfiletype.eq.1) then     !gmsh file
             tempfilename=trim(trim(meshfilename) // '_new.msh')
		elseif(meshfiletype.eq.2) then !Gambit neutral file
			 tempfilename=trim(trim(meshfilename) // '_new.neu')
		else
			 write(*,*) 'Mesh file type not supported!'
			 stop
        endif
        
        reloadfile=  trim(trim(meshfilename)//'.msh')
         ...
         open(2, file = reloadfile, status = 'unknown', iostat = ios)
         

		 !write out 4 line of comments
         read(2,'(A)')  text
		 write(1,'(A)') ADJUSTL(trim(text))
         read(2,'(A)')  text
		 write(1,'(A)') ADJUSTL(trim(text))
         read(2,'(A)')  text
		 write(1,'(A)') ADJUSTL(trim(text))
		 ...
		 do i = 1, nNodes
            read(2,'(A)')  text
			write(1, '(I10, F15.5, F15.5, F12.5)') i,pcoor(i,1),pcoor(i,2),pcoor(i,3)
		 enddo
		...
		if(meshfiletype.eq.1) then     !gmsh file
			 tempfilename=trim(trim(meshfilename) // '.msh')
		elseif(meshfiletype.eq.2) then !Gambit neutral file
			 tempfilename=trim(trim(meshfilename) // '.neu')
		else
			 write(*,*) 'Mesh file type not supported!'
			 stop
		endif
